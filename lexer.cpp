...
string toString(enum TokenType type){
    switch(type){
...
vector<Token> tokenize(const string& input, int line) {
    vector<Token> tokens;
...
/*
int main(){
    string input = "int 11x; \nhello = 12";
    vector<Token> tokens = tokenize(input, 1);
    for (const auto& token : tokens){
        cout << token.value << " " << toString(token.type) << " " << token.line << " ";
    }
    return 0;
}
 */